...
def main():
    ...
    max_weight_value = max(np.max(np.abs(w)) for layer in orig_model.layers for
                           w in layer.get_weights())
    if max_weight_value > SAFE_FLOAT_RANGE:
        print(f"Warning: {SAFE_FLOAT_RANGE=} does not cover the full range of"
              f"weight value ({max_weight_value})")
